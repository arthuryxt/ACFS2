...
for(my $i=1; $i<$Nr; $i++) {
    ...
    if (-e $filein) {
        ...
        while (<IN>) {
            ...
            my @a=split("\t",$_);
            if (scalar(@a) < 23) {
                die "Input file $filein is not expected. Please use the \"\*\.sum\" files generated by ACF_step1\.pl or get_circRNA_from_bed\.pl";
            }
            #if ($a[2]=~m/^chromosome/i) {$a[2]=~s/chromosome//i;}
            #if ($a[2]=~m/^chr/i) {$a[2]=~s/chr//i;}
            my $id=join("_",$a[2],$a[6],$a[10],$a[18].abs($a[10] - $a[6]));
            if (exists $uniq{$id}) {
                my @b=split("\t",$uniq{$id});
                my $seqid=$a[0];
                if ($b[-1]=~m/$seqid/) {
                ...
                $uniq{$id}=join("\t",$a[2],$a[6],$a[10],($a[10] - $a[6]),$a[15],$a[16],$a[17],$a[18],$a[19],$a[20],$a[21],$a[22],$a[23],$a[0]);
            }
        }
        close IN;
    }
