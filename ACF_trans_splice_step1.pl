...
my $genome=$ARGV[2];    # /data/iGenome/mouse/Ensembl/NCBIM37/Sequence/WholeGenome/
my $fileout=$ARGV[3];
my $cutoff=0.9;
...
foreach my $id (keys %READ) {
    ...
    if ($a[9] eq "-") {
		...
		if ($a[15] eq "-") {
			$end=$a[14];
			$left_seq=substr($CHRSEQ{$chr2},($end-2*$Extend-1),(4*$Extend + 1));
            ...
            $right_seq=substr($CHRSEQ{$chr1},($start-2*$Extend-1),(4*$Extend + 1));
