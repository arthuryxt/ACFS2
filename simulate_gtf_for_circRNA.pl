...
my $minExonLen=10;
if (scalar(@ARGV) > 3) { $minExonLen=$ARGV[3]; }
my %uniq;
my %biotype;
my %Gname;
my %ExonCnt;
	...
	if ($ExonCnt{$id} > 2) {
		...
		if ($cnt eq 3) {
			...
			if (exists $circ{$circ_id}) {}
                ...
                my $tmpExonLen=$End[1]-$Start[1];
                if ($tmpExonLen < $minExon) { $minExon = $tmpExonLen; }
			}
            $MinExonLen{$circ_id}=$minExon;
		}
		elsif ($cnt > 3) {
			...
			for(my $trail=0; $trail<$GenerateNr; $trail++){
				...
				if ((0 < $min) and ($min < $max) and ($max < ($cnt-1))) {
					...
					if (exists $circ{$circ_id}) { } #$trail--; }
                        ...
                        my $tmpExonLen=$End[$min]-$Start[$min];
                        if ($tmpExonLen < $minExon) { $minExon = $tmpExonLen; }
